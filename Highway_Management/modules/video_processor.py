...
class VideoProcessor:
    ...
    def setup(self):
        ...
        self.ocr_model = PaddleOCR(
            lang="en", show_log=False, use_angle_cls=True, use_gpu=False
        )
        ...
        width, height = self.video_info.resolution_wh
        ...
        # Create instance of BoxAnnotator and LineZoneAnnotator
        self.box_annotator = BoxAnnotator(
            color=ColorPalette.DEFAULT, thickness=4, text_thickness=4, text_scale=1
        )
        ...
        self.plate_recognizer = PlateRecognizer(
            license_plate_detector=self.plate_detector, ocr_model=self.ocr_model
        )
        self.speed_estimator = SpeedEstimator(
            cfg=self.config["camera"], fps=self.video_info.fps
        )
            ...
            for frame in tqdm(self.generator, total=self.video_info.total_frames):
                ...
                for x1, y1, x2, y2, conf, id in results[0].boxes.data.cpu().numpy():
                    if id in self.CLASS_ID:
                        dets.append(
                            ([int(x1), int(y1), int(x2 - x1), int(y2 - y1)], conf, id)
                        )
                ...
                if dets:
                    ...
                    # mask = [conf is not None for conf in track_confs]
                    ...
                    if len(track_dets) > 0:
                        ...
                        detections = detections[detections.confidence != None]
                        ...
                        for (xyxy, _, confidence, class_id, tracker_id, _) in detections:
                            ...
                            if xyxy[1] > self.y_min:
                                ...
                                if step == 0:
                                    ...
                                    box_vel = np.array(data_deque[0]) - np.array(
                                        data_deque[1]
                                    )

                                    # Calculate the new speed
                                    speed = self.speed_estimator.compute_vel(
                                        xyxy,
                                        box_vel,
                                        step,
                                        frame_num,
                                    ...
                                    # Update speed label
                                    speed_label = (
                                        f" | Speed: {tracker_state[0]:0.2f} mph"
                                    )

                                # Recognize plate
                                plate, ocr_conf = self.plate_recognizer.detect(
                                    frame, xyxy
                                )
                                if plate is not None and ocr_conf > tracker_state[-1]:
                                    tracker_state[-2], tracker_state[-1] = (
                                        plate,
                                        ocr_conf,
                                    )
                                ...
                                if tracker_state[-2] is not None:
                                    ...
                                    frame = self.plate_recognizer.annotate(
                                        frame, xyxy, text
                                    )
                            ...
                            if confidence is not None:
                                ...
                                labels.append(
                                    f"[{tracker_id}] {self.CLASS_DICT[class_id].capitalize()} "
                                    f"| Conf: {confidence}" + speed_label
                                )

                            # Annotate tracking trail
                            self.trace_annotator.annotate(
                                frame, tracker_state[3], class_id
                            )
